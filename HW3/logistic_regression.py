...
class LogisticRegression(object):
	...
	def bias_augment(self, x: np.ndarray) ->np.ndarray:
		...
		"""
		N, _ = x.shape
		# return np.insert(x, 0, np.full((N, 1), 1), axis=1)
		return np.hstack((np.full((N, 1), 1), x))
	...
	def predict_labels(self, h_x: np.ndarray, threshold: float) ->np.ndarray:
		...
		N, D = h_x.shape
		return np.array([1 if prob > threshold else 0 for prob in h_x]).reshape(N, 1)
	...
	def loss(self, y: np.ndarray, h_x: np.ndarray) ->float:
		...
		Return:
			loss (float)
		"""
		# return -np.sum(y * np.log2(h_x).T + (1 - y) * np.log2(1 - h_x).T)
		
		h_x = np.clip(h_x, 1e-15, 1) # prevent log(0) by constraining min value to 1e-15 and max value to 1
	...
	def fit(self, x_train: np.ndarray, y_train: np.ndarray, x_val: np.
		...
		"""
		N, D = x_train.shape
		# theta = np.vstack(([1], np.zeros((D, 1))))\
		theta = np.zeros([D + 1, 1])

		x_train_aug = self.bias_augment(x_train)

		for epoch in range(epochs):
			h_x = self.predict_probs(x_train_aug, theta)  # shape (N, 1)
		
			grad = self.gradient(x_train_aug, y_train, h_x)  # shape (D+1, 1)
