...
def cluster_pixels_gmm(image, K, max_iters=10, full_matrix=True):
	...
	# dataset = image.reshape(-1, 3).astype(np.float64) # H * W gets resized to fit within the first dimension, hence the -1. Dimensionality D = 3 remains unchanged

	gmm_model = GMM(dataset, K, max_iters)
	tau, (_, mu, _) = gmm_model.__call__(full_matrix)
	
	# Assign each pixel to the cluster with the highest responsibility
	cluster_assignments = np.argmax(tau, axis=1)  # Shape: (N,)
	
	# Map each pixel to its cluster's mean
	clustered_dataset = mu[cluster_assignments]  # Shape: (N, 3)
	
	# clustered_dataset = np.clip(clustered_dataset, 0, 255).astype(np.uint8)  

	# Step 5: Reshape the clustered dataset back to the original image shape
	clustered_img = clustered_dataset.reshape(height, width, 3)  # Convert back to uint8
